...
async function action(req, res, next)
{
    try
    {
        console.log('File to upload: ' + req.file.path);
        console.log('Where to upload: ' + config.cloudDir + '\\' + req.file.originalname);
        await fs.rename(req.file.path, config.cloudDir + '/' + req.file.originalname);
        res.redirect(303, '/cloud/');
    }
    catch(err)
    {
        res.render('error.hbs', { message: 'The file provided does not exist.' })
